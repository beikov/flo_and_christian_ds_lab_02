...
public final class PasswordUtils {
    ...
    private static String bytesToHex(byte[] bytes) {
        ...
        for(int i = 0; i < bytes.length; i++) {
            hex.append(HEXES.charAt((bytes[i] & 0xF0) >> 4))
               .append(HEXES.charAt((bytes[i] & 0x0F)));
        }
        
        return hex.toString();
    }
    private static String getHashHexString(String text, String algo, String salt, int iterations){
        try {
