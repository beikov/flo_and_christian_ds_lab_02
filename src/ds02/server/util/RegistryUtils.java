	...
	static {
		...
		public static <T extends Remote> T getRemote(Class<T> clazz) {
			try {
				...
				LOG.error("Could not lookup Remote Object of type " + clazz.getName(), e);
			}
			return null;
		}
		
		public static <T extends Remote, U extends T> void bindService (Class<T> clazz, U remote) {
			try {
				REMOTE_SET.put(clazz.getName(), STUB);
				getRegistry().bind(clazz.getName(), UnicastRemoteObject
